...
export interface HomeState {
  ...
  setMode: (mode: "qa" | "paragraphs" | "keywords" | "sections" | "semantic" | "topics" | "hierarchical" | "entities" | "questions" | "arguments" | "concepts" | "hybrid" | "temporal" | "sentiment" | "dialogue" | "texttiling" | "c99" | "changepoint" | "graph" | "layout") => void;
  keywords: string[];
  ...
  modeFilter: "all" | "qa" | "paragraphs" | "keywords" | "sections" | "semantic" | "topics" | "hierarchical" | "entities" | "questions" | "arguments" | "concepts" | "hybrid" | "temporal" | "sentiment" | "dialogue" | "texttiling" | "c99" | "changepoint" | "graph" | "layout";
  setModeFilter: (filter: "all" | "qa" | "paragraphs" | "keywords" | "sections" | "semantic" | "topics" | "hierarchical" | "entities" | "questions" | "arguments" | "concepts" | "hybrid" | "temporal" | "sentiment" | "dialogue" | "texttiling" | "c99" | "changepoint" | "graph" | "layout") => void;
...
export function useHomeOperations(
  ...
  }, [executeFetch, setUploads, setStatus, state.documentId, state.setDocumentId]);
  ...
  const doUpload = useCallback(async () => {
    ...
    try {
      ...
      if (newDocumentId) {
        trackDocument("upload", newDocumentId);
      }
  ...
  const segmentDoc = useCallback(async () => {
    ...
    if (mode === "concepts" && concepts.length === 0) {
      setStatus("Please add at least one concept or use AI discovery before segmenting.");
      return;
    }
    ...
    try {
      // Use timeout for long-running segmentation (5 minutes)
      const { withTimeout, TIMEOUT_CONFIG } = await import("../../lib/edgeCases");
      const data = await withTimeout(
        segmentDocument(
          documentId, 
          mode,
          mode === "keywords" ? keywords : undefined,
          undefined,
          undefined,
          undefined,
          undefined,
          undefined,
          mode === "concepts" ? concepts : undefined
        ),
        TIMEOUT_CONFIG.upload // Use upload timeout (5 minutes) for segmentation
      );
      
      await loadSegmentationSummary(documentId);
      trackDocument("segment", documentId);
      ...
      if (e?.message?.includes("timeout")) {
        setStatus("Segmentation is taking longer than expected. Please check the document size and try again.");
      } else {
  ...
  const loadSegments = useCallback(async () => {
    ...
    try {
      const response = await listSegments(documentId, mode as any);
  ...
  const deleteSelectedUpload = useCallback(async () => {
    ...
    try {
      ...
      if (selectedUpload.documentId) {
        trackDocument("delete", selectedUpload.documentId);
      }
      setQuery("");
      setModeFilter("all");
      await fetchUploads();
