...
export interface SegmentOperations {
  ...
  loadSegs: (m?: SegmentationMode) => Promise<void>;
...
export function useSegmentOperations(
  ...
  const saveChunkEdit = useCallback(async () => {
    ...
    try {
      ...
      try {
        await setSegmentFolder(docId, updated.id, state.chunkEditFolderId);
      } catch {
        // ignore
      }
      loadFolderMap(docId)
        .then((map) => state.setFolderMap(map))
        .catch(() => {
          // ignore
        });
