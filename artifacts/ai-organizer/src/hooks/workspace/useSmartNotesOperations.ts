...
export function useSmartNotesOperations(
  ...
  const createNewSmartNote = useCallback(() => {
    ...
    state.setSmartNotePriority("medium");
  ...
  const loadSmartNoteForEdit = useCallback(
    (note: SmartNote) => {
      ...
      state.setSmartNotePriority(note.priority || 'medium');
  ...
  const saveSmartNoteLocal = useCallback(async () => {
    try {
      ...
      if (state.currentSmartNote) {
        ...
        // Create new note
        const newNote = await saveSmartNote(
          {
            content: state.smartNoteText,
            html: state.smartNoteHtml,
            tags: state.smartNoteTags,
  ...
  const deleteSmartNoteLocal = useCallback(
    async (noteId: string) => {
      if (window.confirm("Delete this Smart Note? This cannot be undone.")) {
        try {
          await deleteSmartNote(docId, noteId);
          const notes = await loadSmartNotes(docId);
          state.setSmartNotes(notes);
          ...
          state.setSmartNoteStatus(`Failed to delete: ${error?.message || String(error)}`);
        }
      }
    },
  ...
  const filteredSmartNotes = useMemo(() => {
    ...
    if (state.smartNoteSelectedPriority !== "all")
      filtered = filterSmartNotesByPriority(filtered, state.smartNoteSelectedPriority);
    // Apply sorting
    filtered = sortSmartNotes(filtered, state.smartNoteSortBy);
    return filtered;
  }, [
    state.smartNotes,
    ...
    state.smartNoteSelectedPriority,
    state.smartNoteSortBy,
  ]);
