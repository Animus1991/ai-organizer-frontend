...
export function useFileUpload() {
  ...
  const upload = useCallback(async (file: File): Promise<any> => {
    ...
    try {
      // Auto-hash file before upload for integrity verification
      const { hashFile } = await import('../services/blockchain/HashService');
      const { recordAuditEntry } = await import('../services/blockchain/AuditTrailService');
      
      const fileHash = await hashFile(file);
      console.log(`[Blockchain] Pre-upload hash: ${fileHash.hash}`);

      // Record audit entry for document upload
      const actor = localStorage.getItem('aiorg_username') || 'anonymous';
      await recordAuditEntry('DOCUMENT_UPLOADED', actor, {
        filename: file.name,
        fileSize: file.size,
        fileType: file.type,
        preUploadHash: fileHash.hash,
