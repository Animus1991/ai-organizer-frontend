...
export default function FolderView({ docId, folder, onBack, onChunkUpdated }: FolderViewProps) {
  ...
  useEffect(() => {
    const loadItems = async () => {
      try {
        ...
        const folderWithItems = await apiGetFolder(typeof folder.id === 'string' ? parseInt(folder.id, 10) : folder.id, true);
  ...
  const handleSaveTitle = async (chunkId: string) => {
    ...
    Promise.resolve(onChunkUpdated()).catch((err: unknown) => console.error("onChunkUpdated error:", err));
  };
