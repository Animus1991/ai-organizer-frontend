...
export async function withTimeout<T>(
  ...
  // When timeout occurs, the timeout promise rejects and Promise.race rejects
  // The original promise continues but won't cause issues since Promise.race already resolved/rejected
  try {
    ...
    // If timeout occurred, ensure the timeout promise's rejection is handled
    // This prevents unhandled rejection warnings
    timeoutPromise.catch(() => {
      // Timeout rejection is expected - this catch prevents unhandled rejection warnings
    });
    throw error;
  }
}
