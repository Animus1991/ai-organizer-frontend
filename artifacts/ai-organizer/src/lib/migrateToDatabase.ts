...
export async function migrateDocumentToDatabase(documentId: number): Promise<{
  ...
  try {
    ...
    const folders = await loadFolders(documentId);
    const folderMap = await loadFolderMap(documentId);
    const duplicatedChunks = loadDuplicatedChunks(documentId);
    const smartNotes = await loadSmartNotes(documentId);
