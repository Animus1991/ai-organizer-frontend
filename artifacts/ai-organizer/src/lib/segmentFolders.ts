...
function _apiToLegacyFolder(apiFolder: FolderDTO, items?: FolderItemDTO[]): FolderDTO & { contents: string[] } {
  ...
  return {
    ...
    children: apiFolder.children ? apiFolder.children.map(child => _apiToLegacyFolder(child)) : undefined,
  } as any;
}
...
export async function loadFolders(docId: number, skipCache: boolean = false): Promise<FolderDTO[]> {
  ...
  try {
    ...
    // Convert to legacy format for backward compatibility, preserving nested structure
    function convertFolder(folder: FolderDTO): FolderDTO & { contents: string[] } {
      return {
        id: String(folder.id),
        name: folder.name,
    ...
    return apiFolders.map(convertFolder);
  } catch (error) {
 ...
 * P3: Supports nested folders via parentId parameter
 */
export async function createFolder(docId: number, name: string, parentId?: number | null): Promise<FolderDTO> {
  await ensureMigrated(docId);
  
  try {
    const folder = await apiCreateFolder(docId, name, parentId);
...
export function filterFoldersWithItems(
  ...
  return folders.filter(folder => {
    ...
    return Object.values(folderMap).some(mappedFolderId => String(mappedFolderId) === String(folderId));
  });
}
