...
/**
 * Filter smart notes by priority
 */
export function filterSmartNotesByPriority(notes: SmartNote[], priority: string): SmartNote[] {
  if (priority === 'all') return notes;
  return notes.filter(note => note.priority === priority);
}

/**
 * Sort smart notes by various criteria
 */
export type SortOption = 'date-desc' | 'date-asc' | 'category' | 'priority' | 'title';

export function sortSmartNotes(notes: SmartNote[], sortBy: SortOption): SmartNote[] {
  const sorted = [...notes];
  ...
  switch (sortBy) {
    ...
    case 'category':
      return sorted.sort((a, b) => a.category.localeCompare(b.category) || new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
    case 'priority':
      const priorityOrder = { high: 3, medium: 2, low: 1 };
      return sorted.sort((a, b) => {
        const aPriority = priorityOrder[a.priority || 'medium'] || 2;
        const bPriority = priorityOrder[b.priority || 'medium'] || 2;
        return bPriority - aPriority || new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime();
      });
    case 'title':
      return sorted.sort((a, b) => {
        const aTitle = a.content.substring(0, 50).toLowerCase();
        const bTitle = b.content.substring(0, 50).toLowerCase();
